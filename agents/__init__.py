...
@tool
def analyze_time_patterns(client_id: str) -> Dict[str, Any]:
    """
...
def create_sample_data():
    ...
    sample_data = []
    client_ids = ['12345', '67890', '11111']
    mcc_categories = ['Grocery', 'Dining', 'Gas', 'Entertainment', 'Shopping', 'Healthcare']
    merchants = ['Store_A', 'Store_B', 'Restaurant_X', 'Gas_Station_1', 'Cinema_Y']
    ...
    for i in range(1000):
        ...
        sample_data.append({
            ...
            'mcc_category': random.choice(mcc_categories),
            'merchant_id': random.choice(merchants),
            'merchant_city': random.choice(['New York', 'Los Angeles', 'Chicago']),
            'merchant_state': random.choice(['NY', 'CA', 'IL']),
            'is_weekend': 1 if date.weekday() >= 5 else 0,
            'day_name': date.strftime('%A'),
            'txn_hour': random.randint(6, 23),
            'is_night_txn': 1 if random.randint(6, 23) >= 20 else 0,
            'card_id': f'card_{random.randint(1, 5)}',
...
def demo_spending_agent():
    ...
    print("\n🤖 Initializing Spending Agent...")
    agent = SpendingAgent('sample_banking_data.csv', model_name="gpt-4")
    ...
    test_queries = [
        ...
        "Where do I spend most of my money by category?",
        "Do I spend more on weekends compared to weekdays?",
        "What are my top merchants and where do I shop most?",
        "Find any unusual or suspicious transactions in my account",
        "Compare my current month spending to last month"
    ]
    
    client_id = "12345"
    
    print(f"\n🔍 Testing Queries for Client {client_id}")
    print("=" * 60)
    ...
    for i, query in enumerate(test_queries, 1):
        ...
        # Process query
        result = agent.process_query(client_id, query)
        ...
        if result.get('error'):
            ...
            print(f"💬 Response: {result.get('response', 'No response generated')[:200]}...")
...
def test_individual_tools():
    ...
    create_sample_data()
    data_store = DataStore()
    data_store.load_data('sample_banking_data.csv')
    
    client_id = "12345"
    
    # Test each tool
    tools_to_test = [
        (get_spending_summary, {"client_id": client_id, "period": "all"}),
        (analyze_spending_by_category, {"client_id": client_id, "top_n": 5}),
        (analyze_time_patterns, {"client_id": client_id}),
        (analyze_merchant_patterns, {"client_id": client_id, "top_n": 5}),
        (detect_spending_anomalies, {"client_id": client_id, "sensitivity": "medium"})
    ]
    ...
    for tool, params in tools_to_test:
        ...
        try:
            ...
            print(f"✅ Success: {type(result)} with {len(result)} keys")
